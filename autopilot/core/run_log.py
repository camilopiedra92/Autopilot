...
class RunLogProtocol(abc.ABC):
    ...
    @abc.abstractmethod
    async def get_run(self, workflow_id: str, run_id: str) -> WorkflowRun | None:
    ...
    @abc.abstractmethod
    async def delete_run(self, workflow_id: str, run_id: str) -> bool:
        """Delete a run from history. Returns True if deleted, False if not found."""
        ...
...
class InMemoryRunLogService(RunLogProtocol):
    ...
    async def save_run(self, run: WorkflowRun) -> None:
        with tracer.start_as_current_span(
            "run_log.save",
            attributes={"workflow_id": run.workflow_id, "run_id": run.id},
        ):
    ...
    async def delete_run(self, workflow_id: str, run_id: str) -> bool:
        with tracer.start_as_current_span(
            "run_log.delete", attributes={"workflow_id": workflow_id, "run_id": run_id}
        ):
            wf_runs = self._runs.get(workflow_id, {})
            run = wf_runs.get(run_id)
            if not run:
                return False
            # Adjust stats
            self._stats[workflow_id]["total"] = max(
                0, self._stats[workflow_id]["total"] - 1
            )
            if run.status == RunStatus.SUCCESS:
                self._stats[workflow_id]["successful"] = max(
                    0, self._stats[workflow_id]["successful"] - 1
                )
            del wf_runs[run_id]
            return True
...
class FirestoreRunLogService(RunLogProtocol):
    ...
    def _run_ref(self, workflow_id: str, run_id: str):
        ...
        return self._workflow_ref(workflow_id).collection("runs").document(run_id)
    ...
    async def get_pending_runs(self) -> list[WorkflowRun]:
        with tracer.start_as_current_span("run_log.get_pending_runs"):
            query = (
                self.client.collection_group("runs")
                .where(field_path="status", op_string="==", value="paused")
                .order_by("started_at", direction="DESCENDING")
            )
    ...
    async def delete_run(self, workflow_id: str, run_id: str) -> bool:
        with tracer.start_as_current_span(
            ...
            run_ref = self._run_ref(workflow_id, run_id)
            wf_ref = self._workflow_ref(workflow_id)

            @fs.async_transactional
            async def _delete_with_stats(transaction):
                doc = await run_ref.get(transaction=transaction)
                if not doc.exists:
                    return False

                run_data = doc.to_dict()
                transaction.delete(run_ref)
                ...
                current["total"] = max(0, current.get("total", 0) - 1)
            ...
            transaction = self.client.transaction()
            return await _delete_with_stats(transaction)
