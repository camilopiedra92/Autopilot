...
8.  **Artifacts**: No action needed. All three execution engines (`Pipeline`, `DAGRunner`, `ADKRunner`) automatically persist each step/node output as versioned JSON artifacts. In production with `ARTIFACT_BACKEND=gcs`, artifacts are saved to GCS under `{app_name}/default/{execution_id}/`. See `ARCHITECTURE.md § Automatic Artifact Persistence`.
9.  **Register Tools** (if workflow uses external APIs or reusable functions):
...
10. **Create Agents** (optional, for Full level):
...
11. **Notify User**: Inform the user the workflow is created. Remind them to restart the platform for auto-discovery.
